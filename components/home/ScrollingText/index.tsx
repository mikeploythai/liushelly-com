...
export default async function ScrollingText() {
  ...
  return (
    <section className="flex w-full justify-center bg-brand-dark dark:bg-brand-light">
      <div className="group flex relative max-w-screen-lg w-full p-4 gap-2 overflow-hidden">
        <ScrollingTextList data={data} />
        <ScrollingTextList data={data} />
        <span className="absolute top-0 left-0 h-full w-1/5 bg-gradient-to-r from-brand-dark to-transparent dark:from-brand-light" />
        <span className="absolute top-0 right-0 h-full w-1/5 bg-gradient-to-l from-brand-dark to-transparent dark:from-brand-light" />
      </div>
    </section>
  );
}
